...
| `docs/*.html`、`html/`、`latex/` | **生成物，不进版本库**（见仓库根目录 `.gitignore`） |

## 一、本地生成

必须在**工程根目录**执行（Doxyfile 里 `INPUT` / `OUTPUT_DIRECTORY` 是相对路径，本地与 CI 行为一致）：
...
1. 安装 Graphviz（生成包含关系图）；下载 **`env.DOXYGEN_VERSION`** 指定版本的 Doxygen **官方二进制**
...
首次使用需在仓库 **Settings → Pages → Source** 选择 **GitHub Actions**（一次性；用 Actions 部署时
不需要 `.nojekyll`，工作流里仍会创建一个作为兜底，以便将来改用"按分支托管"）。

## 三、PDF 与离线包（按需）

- **本地 PDF**：`doxygen/latex/` 会保留，进入该目录执行 `make.bat`（Windows）或 `make`（Linux/macOS，
  需已安装 LaTeX 发行版）即可得到 `refman.pdf`。**CI 里用 `GENERATE_LATEX = NO` 关掉了这一步**，
  纯粹是为了省构建时间。
- **离线 HTML 包**：仓库根目录的 `StepMotorCtrl_42_API_Doc_html.zip` 是本地打好的离线包
  （解压后双击 `index.html`，含中文阅读说明，已加入 `.gitignore`），适合直接发给别人。
- **将来要"一个文件"分发**：可以再加一个 job——在 CI 里生成 PDF 或 HTML 压缩包，并挂到
  **GitHub Release** 附件上；这样在线看网站、离线取单文件，两条路都有。
