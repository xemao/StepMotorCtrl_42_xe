...
>
> **复核记录**:2026-09-18 依据源码逐条核对,修正了 ADC 电流反馈、CAN 多机通信、PID/DCE 角色、"三环闭环"等与代码不符的描述。
...
- 通信:USART1(上位机命令 + 遥测);CAN1 只做了初始化,收发逻辑未实现。
...
```
 ...
 USART 上位机    ─DMA──► 命令解析(uart_cmd)      │
        ...
        │ 20kHz:位置环(DCE) / 速度环(PID) / 指令电流直接输出 │
        └──────────────────────────┬────────────────────────┘
                                   ▼
                    ┌────────────── 输出 ──────────────┐
...
**这是一条反馈控制回路(位置/速度):** 编码器 → 20kHz 闭环(位置用 DCE、速度用 PID) → PWM → 驱动芯片 → 电机,再回到编码器。

> **注意(与代码的差异,已核对源码):**
> - **没有电流反馈**:ADC1 只被 CubeMX 初始化(`MX_ADC1_Init`),全工程没有 `HAL_ADC_Start_*` / `HAL_ADC_GetValue` 调用;
>   `s_foc_current` 是控制器算出的**指令电流**,经 TB67H450 开环输出。所以本项目是"位置/速度闭环 + 电流矢量开环",
>   并非三环闭环。
> - **CAN 未实现**:`MX_CAN_Init` 只做了初始化(900 kbit/s),没有收发逻辑;`boardConfig.canNodeId` 仅被存储、无人读取。
...
| `motor.c` | 电机系统核心:模式切换、位置环(DCE)/速度环(PID)、指令电流矢量输出、堵转与过载检测 |
...
**控制结构**:`PID_t`(速度环,用于 `MODE_COMMAND_VELOCITY`)+ `DCE_t`(位置环,位置与速度误差合并成一次定点运算,用于
`MODE_COMMAND_POSITION`)——见 `Controller_Config_t`。电流没有独立控制器,也没有实测反馈。
...
> 上电后 `main()` 读 Flash 配置 → 初始化电机系统 → 启动 20kHz 定时器;之后每个 50µs,`TIM4` 中断里跑**编码器校准状态机**或**电机闭环**(位置用 DCE、速度用 PID,输出的量是**指令电流**),算出的 FOC 电流矢量经 `TB67H450` 输出到电机绕组;与此同时,磁编码器通过 SPI 反馈角度、上位机通过串口发命令改目标、按键和 LED 在 100Hz 中断里处理人机交互。
